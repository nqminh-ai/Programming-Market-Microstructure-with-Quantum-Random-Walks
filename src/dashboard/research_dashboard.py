...
def run_crw(n_steps: int, n_paths: int = 5000):
    ...
    unique_positions, position_counts = np.unique(final_positions, return_counts=True)
    counts[np.searchsorted(pos_range, unique_positions)] = position_counts
    prob = counts / n_paths
...
def sensitivity_heatmap(alpha_val: float, coin_type: str, n_steps: int):
    ...
    for gamma_idx, gamma_value in enumerate(gammas):
        for alpha_idx, alpha_value in enumerate(alphas):
            _, _, var_list = run_qrw(gamma_value, alpha_value, coin_type, n_steps)
            z[gamma_idx, alpha_idx] = var_list[-1] / crw_var if crw_var > 0 else 0.0
...
def main() -> None:
    ...
    st.markdown(
        ...
        "Quantum Random Walk — Market Microstructure</h1>",
