...
These cover the event-extraction and fair-baseline primitives that every Phase
1-6 research script imports (``alpha_phase_ablation``,
``strong_baseline_comparison``, ``full_dataset_confirmation``). The file was
previously disabled as a corrupt ``.bak``, which left those primitives with no
coverage at all.

Two further tests existed for ``walk_forward_boundaries`` and
``walk_forward_verdict``; both functions were folded into
``walk_forward_evaluation`` during a later refactor, so they are not restored
here rather than being rewritten against an API that no longer exists.
"""
